...
class EncoderLSTM(nn.Module):
    def __init__(self, opt):
        ...
        # far points
        self.lstm0 = nn.LSTM(self.cnn_dim, self.hidden_dim, self.layer_dim, bidirectional=True, batch_first=True)
        self.lstm1 = nn.LSTM(self.cnn_dim, self.hidden_dim, self.layer_dim, bidirectional=True, batch_first=True)
        self.lstm2 = nn.LSTM(self.cnn_dim, self.hidden_dim, self.layer_dim, bidirectional=True, batch_first=True)
        # distribution_indicator
        self.lstm3 = nn.LSTM(1, self.hidden_dim, self.layer_dim, bidirectional=True, batch_first=True)
        # near points
        self.lstm4 = nn.LSTM(1, self.hidden_dim, self.layer_dim, bidirectional=True, batch_first=True)
